...
#include <stdlib.h>
#include <string.h>
#include "fix_cylinder_reflect.h"
#include "atom.h"
#include "comm.h"
#include "update.h"
#include "modify.h"
...
FixCylinderReflect::FixCylinderReflect(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  nwall(0)
{
  if (narg < 4) error->all(FLERR,"Illegal fix cylinder/reflect command");
  ...
  while (iarg < narg) {
    if ((strcmp(arg[iarg],"xlo") == 0) || (strcmp(arg[iarg],"xhi") == 0) ||
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix cylinder/reflect command");
      ...
      for (int m = 0; (m < nwall) && (m < 6); m++)
        if (newwall == wallwhich[m])
          error->all(FLERR,"Wall defined twice in fix cylinder/reflect command");
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal cylinder/reflect command");
      if (strcmp(arg[iarg+1],"box") == 0) scaleflag = 0;
      else if (strcmp(arg[iarg+1],"lattice") == 0) scaleflag = 1;
      else error->all(FLERR,"Illegal fix cylinder/reflect command");
      iarg += 2;
    } else error->all(FLERR,"Illegal fix cylinder/reflect command");
  }
  ...
  for (int m = 0; m < nwall; m++) {
    if ((wallwhich[m] == XLO || wallwhich[m] == XHI) && domain->xperiodic)
      error->all(FLERR,"Cannot use fix cylinder/reflect in periodic dimension");
  ...
  for (int m = 0; m < nwall; m++)
    if ((wallwhich[m] == ZLO || wallwhich[m] == ZHI) && domain->dimension == 2)
      error->all(FLERR,
                 "Cannot use fix cylinder/reflect zlo/zhi for a 2d simulation");
...
FixCylinderReflect::~FixCylinderReflect()
{
  if (copymode) return;
...
int FixCylinderReflect::setmask()
{
  int mask = 0;
  mask |= POST_INTEGRATE;
...
void FixCylinderReflect::init()
{
  for (int m = 0; m < nwall; m++) {
    if (wallstyle[m] != VARIABLE) continue;
    varindex[m] = input->variable->find(varstr[m]);
    if (varindex[m] < 0)
      error->all(FLERR,"Variable name for fix cylinder/reflect does not exist");
    if (!input->variable->equalstyle(varindex[m]))
      error->all(FLERR,"Variable for fix cylinder/reflect is invalid style");
  }
...
void FixCylinderReflect::post_integrate()
{
  int i,dim,side;
  double coord;
